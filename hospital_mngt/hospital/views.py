...
def Index(request):
    ...
    appointment=Appoinement.objects.all()
    d = 0
    p = 0
    a = 0
    for i in doctors:
...
def Add_Appointment(request):
    ...
    if request.method=="POST":
        ...
        try:
            Appoinement.objects.create(Doctor=doctor,  Patient=patient, date=da,time=t)
            error ='no'
        except:  
            error='yes' 
...
def Delete_Appointment(request, pid):
    if not request.user.is_staff:
        return redirect('login') 
    app= Appoinement.objects.get(id=pid)
    app.delete()
